...
class TestVisitedList(TestCase):
    ...
    def test_visited_list_contains_only_visited_places(self):
...
class TestAddNewPlace(TestCase):
    ...
    def test_add_new_unvisited_place(self):
        ...
        self.assertEqual(1, len(response_places)) # Check only one place added
        
        tokyo = response_places[0]
        tokyo_from_database = Place.objects.get(name='Tokyo', visited=False)

        self.assertEqual(tokyo, tokyo_from_database)
